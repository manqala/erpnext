...
from cpfa.utils import get_pdf_link
...
class WorkflowNotification(Document):
	pass
...
def process_workflow_actions(doc, state):
	...
	clear_doctype_notifications(frappe.new_doc('Workflow Notification'))
...
def update_completed_workflow_actions(doc, user=None):
	user = user if user else frappe.session.user
	frappe.db.sql("""UPDATE `tabWorkflow Notification` SET `status`='Completed', `completed_by`=%s
		WHERE `reference_doctype`=%s AND `reference_name`=%s AND `user`=%s AND `status`='Open'""",
...
def filter_allowed_users(users, doc, transition):
	...
	for user in users:
		if (has_approval_access(user, doc, transition)
			and has_permission(doctype=doc.doctype, user=user)):
